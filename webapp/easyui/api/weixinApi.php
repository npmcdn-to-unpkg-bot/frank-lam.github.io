...
function logdebug($text){
   if(class_exists('Log')){
    @Log::write($text,Log::INFO,Log::FILE,LOG_PATH."weixin.log");  
}
}
class Wechat
{
     ...
     * 【已经使用】
	 * 获取消息发送者openid值
	 * 例如：$openid= wxchat->getRevFrom();
	 * 返回数据（text类型）：o1y1ouBhh7ECdgLTqaSjYS3uA9A8   
     */
...
return sprintf($format, $encrypt, $signature, $timestamp, $nonce);
     ...
     * @param int $type 0:临时二维码；   $type 1:永久二维码(此时expire参数无效)；
	...
	/**
	 * 【自增方法】2016-06-02  10:51
	 * 临时增加的函数，方便为了获取临时的二维码
	 * 获取永久二维码
	 * 
	 * @param int|string  $scene_id 自定义追踪id,临时二维码只能用数值型
	 * @param int $type 0:临时二维码；   $type 1:永久二维码(此时expire参数无效)；
	 * @return string qcodeURL
	 */
	function getForeverQcode($scene_id,$type=1){
		$ticketArr=$this->getQRCode($scene_id,$type);
		$ticket=$ticketArr['ticket'];
		$qcodeURL=$this->getQRUrl($ticket);
		return $qcodeURL;
	}
     ...
     * uuid、major、minor:三个�息需填写完整，若填了设备编号，则可不填此信息
     * +-------------------------------------------------------------------------------------------------------------
